const address = 'ec2-52-47-113-242.eu-west-3.compute.amazonaws.com:8443'
const wsc = new WebSocket('wss://' + address );
...
$.get({url: 'https://'+ address +'/api/settings/currenttemp', async: false}, () => {
    console.log('success');
})
.done((data) => {
    console.log('done');
...
$.get({url: 'https://' + address + '/api/settings/heating', async: false}, () => {
        console.log('success');
    })
    .done((data) => {
        console.log('done');
...
$.get({url: 'https://' + address + '/api/settings/cooling', async: false}, () => {
    console.log('success');
})
.done((data) => {
    console.log('done');
    ...
    if(flag == 0) {
        switch(splittedmsg[0]) {
            ...
            case 'cooling': if(splittedmsg[1] == 'on')
                            ...
                            break;
            case 'mode' : case 'mode' : if(splittedmsg[1] == 'man')
                            $('#active_mode').text('manual');
                          else {
                            if(splittedmsg[1] == 'prog')
                                $('#active_mode').text('program');
                            else $('#active_mode').text(splittedmsg[1]);
                          }
                          break;
...
$('#man').on('click', () => {
    const url = 'https://' + address + '/api/settings/mode';
    const data = '{"mode":"man"}';
    $.ajax({
        url: url,
        data: data, 
...
$('#increase').on('click', () => {
    if(flag == 0) {
        ...
        $.get({url: 'https://' + address + '/api/settings/mode', async: false}, () => {
            console.log('success');
        })
        .done((data) => {
          console.log('done');
        ...
        $.get({url:'https://' + address + '/api/settings/manualtemp', async:false}, () => {
            console.log('success');
        })
        .done((data) => {
        console.log('done');
    ...
    if(mode == 'man') {
        ...
        timer = setTimeout(() => {
            ...
            flag = 0;
            //put on manual temp
            const url = 'https://' + address+ '/api/settings/manualtemp';
...
$('#winter').on('click', () => {
    ...
    console.log('Sending settings to backend...');
    //put on season
    const url = 'https://' + address + '/api/settings/season';
